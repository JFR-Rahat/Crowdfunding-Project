...
const getOwnerSuccessfulProject = async function() {
    ...
    try {
        ...
        var passOwnerSuccessfulOngoingProjects = new Array();
        ...
        ownerProjects.map((item)=>{
            ...
            if(parseInt(projectList.projectAmountRaised) >= parseInt(projectList.projectFundingGoal) && (currentTime < projectList.projectEndTime)){
                passOwnerSuccessfulOngoingProjects.push(projectList);
                // console.log("Hello");
            }
            else{
                // console.log("Failed");
        ...
        if(passOwnerSuccessfulOngoingProjects.length == 0){

            passOwnerSuccessfulOngoingProjects.push("Empty");
        }

        $.ajax({
            type: "POST",
            url: "setOwnerSuccessfulOngoingProjects.php",
            data: {'ownerSuccessfulOngoigProjects': JSON.stringify(passOwnerSuccessfulOngoingProjects)},
            // data : {'ownerProjects': },
            success: function(ownerSuccessfulOngoigProjects){
                console.log("Succeed to send ownerProjects: ", passOwnerSuccessfulOngoingProjects);
            }
        });

        window.location = "../client/user/successfulOngoingProject.php";
