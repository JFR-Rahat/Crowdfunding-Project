...
const getOwnerProject = async function() {
    ...
    try {
        ...
        var passOwnerProjects = new Array();
        ...
        ownerProjects.map((item)=>{
            ...
            if(currentTime < parseInt(projectList.projectEndTime)){
                passOwnerProjects.push(projectList);
                console.log("Hello");
            }
            else{
                // console.log("Failed");
        ...
        if(passOwnerProjects.length == 0){

            passOwnerProjects.push("Empty");
        }

        $.ajax({
            type: "POST",
            url: "setOwnerProjects.php",
            data: {'ownerProjects': JSON.stringify(passOwnerProjects)},
            // data : {'ownerProjects': },
            success: function(ownerProjects){
                console.log("Succeed to send ownerProjects: ", passOwnerProjects);
            }
        });
